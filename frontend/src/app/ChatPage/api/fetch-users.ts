...
export async function getVueGeust(
  senderId: string,
  id: string,
  isUser: Boolean,
) {
...
export async function getUser(id: string) {
  try {
    const token = Cookies.get('access_token');
    const res = await axios.get(
      `${process.env.NEXT_PUBLIC_BACK}/user/geust/${id}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
export async function getMembersChannel(senderId: string, id: string) {
  try {
    const token = Cookies.get('access_token');
    const res = await axios.get(
      `${process.env.NEXT_PUBLIC_BACK}/channel/getMembersChannel/${senderId}/${id}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
